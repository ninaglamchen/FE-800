...
def save_price_data(tickers,paths):
    db = get_db()
    db.ETF.delete_many({})  # empty the database

    qd.ApiConfig.api_key = '6opTKxN_Q5d8aGDKKwxm'
    ...
    for k in tickers.keys():
        ...
        # data[k] = pd.Series(temp_data_close)

    data.to_csv(path_or_buf=r"C:\My Files\Study\17 Spring\800 - Special Problems in FE (MS)\Code\FE-800\csv\test.csv")
    return data
